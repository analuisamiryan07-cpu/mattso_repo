...
class GenerateDocumentsRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        $trainings = array_values(array_filter((array) $this->input('capacitaciones', []), 'is_array'));
        $this->merge([
    ...
    public function rules(): array
    {
        ...
        $catalog = new CertificationCatalog((string) $this->input('empresa', 'SAPPER'));
        $province = (string) $this->input('provincia');

        return [
            'empresa' => ['required', Rule::in(array_keys(Client::companies()))],
            'nombre' => ['required', 'string', 'max:180'],
            'cedula' => ['required', new EcuadorIdentification],
            ...
            // El C02 admite explícitamente cero capacitaciones: las tres filas
            // reservadas de la plantilla se generan en blanco.
            'capacitaciones' => ['array', 'max:20'],
