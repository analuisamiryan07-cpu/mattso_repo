...
class VerifyMatssoReadiness extends Command
{
    ...
    private function checkFiles(): void
    {
        ...
        foreach (['SAPPER', 'MATSSO'] as $company) {
            foreach (DocumentGenerationService::templates($company) as $template) {
                $root = $company === 'MATSSO'
                    ? resource_path('document-templates/matsso')
                    : $templateRoot;
                $path = $root.DIRECTORY_SEPARATOR.$template['file'];
                $this->record("Plantilla {$company}/{$template['file']}", is_readable($path), is_readable($path) ? 'Legible' : 'No encontrada');
            }
        }
    ...
    private function checkDatabase(): bool
    {
        try {
            ...
            $contracts = [
                ...
                'clientes' => ['id', 'nombre', 'cedula', 'empresa', 'telefono', 'correo', 'direccion', 'fecha', 'ciudad', 'lugar', 'esquema', 'tipo_examen', 'puntaje_teorico', 'puntaje_practico', 'datos_c02', 'created_at', 'updated_at'],
                'documentos_generados' => ['id', 'cliente_id', 'empresa', 'carpeta', 'zip_ruta', 'fecha_generacion', 'n_archivos', 'nombres_archivos', 'generado_por'],
            ];
            ...
            if ($validContracts['usuarios_admin']) {
                $invalidRoles = DB::table('usuarios_admin')
                    ->whereNotIn('rol', [User::ROLE_ADMIN, User::ROLE_SECRETARY, 'VENDEDOR'])
