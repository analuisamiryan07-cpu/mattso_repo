...
<form method="POST" action="{{ route('documents.store') }}" class="card" id="document-form">
        ...
        <div>
            ...
            <select name="empresa" id="empresa" required>
                ...
                @foreach(\App\Models\Client::companies() as $value => $label)
                    <option value="{{ $value }}" @selected(old('empresa') === $value)>{{ $label }}</option>
                @endforeach
            </select>
        </div>
    </div>
    ...
    <div class="form-grid">
        ...
        <div><label>Cédula o RUC</label><input name="cedula" value="{{ old('cedula') }}" inputmode="numeric" minlength="10" maxlength="13" pattern="(?:[0-9]{10}|[0-9]{10}001)" title="Ingrese 10 dígitos para cédula o 13 para RUC terminado en 001." required></div>
...
(() => {
    ...
    const companyCatalogs = @json($companyCatalogs);
    ...
    function addRepeated(containerId, templateId, data = {}) {
        ...
        card.querySelector('[data-remove]').addEventListener('click', () => {
            ...
            if (container.children.length > minimumItems) {
                card.remove();
    ...
    let examinerData = @json(collect($examiners)->keyBy('id'));
    ...
    function updateCompanyCatalog() {
        const company = document.getElementById('empresa').value || 'SAPPER';
        const catalog = companyCatalogs[company] || companyCatalogs.SAPPER;
        if (!catalog) return;
        const schemeSelects = [...document.querySelectorAll('#applications select[data-scheme]')];
        const selectedSchemes = schemeSelects.map(select => select.value);
        const selectedProfiles = [...document.querySelectorAll('[data-profile]')].map(select => select.value);
        const selectedExaminer = examinerSelect.value;
        const profileOptions = [...new Set(catalog.schemes.map(item => item.profile))];

        document.querySelectorAll('[data-profile]').forEach((select, index) => {
            select.innerHTML = '<option value="">Seleccione un perfil</option>';
            profileOptions.forEach(value => select.add(new Option(value, value)));
            select.value = profileOptions.includes(selectedProfiles[index]) ? selectedProfiles[index] : '';
        });
        schemeSelects.forEach((select, index) => {
            select.innerHTML = '<option value="">Seleccione un esquema</option>';
            catalog.schemes.forEach(item => select.add(new Option(item.scheme, item.scheme)));
            select.value = catalog.schemes.some(item => item.scheme === selectedSchemes[index]) ? selectedSchemes[index] : '';
        });

        examinerData = Object.fromEntries(catalog.examiners.map(examiner => [String(examiner.id), examiner]));
        examinerSelect.innerHTML = '<option value="">Seleccione un examinador</option>';
        catalog.examiners.forEach(examiner => examinerSelect.add(new Option(`${examiner.name} — ${examiner.scheme}`, String(examiner.id))));
        examinerSelect.value = catalog.examiners.some(examiner => String(examiner.id) === String(selectedExaminer)) ? selectedExaminer : '';
        updateExaminerDetails();
    }

    document.getElementById('empresa').addEventListener('change', updateCompanyCatalog);
